...
function Fines() {
  ...
  const fetchFinesSummary = async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/api/fines/summary`, {
        headers: {
  ...
  const fetchFinesList = async () => {
    try {
      ...
      const url = statusFilter
        ? `${API_URL}/api/fines/all?page=${page}&per_page=10&status=${statusFilter}`
        : `${API_URL}/api/fines/all?page=${page}&per_page=10`;
  ...
  const handleMarkPaid = async (fine) => {
    try {
      const token = localStorage.getItem('access_token');
      const response = await fetch(`${API_URL}/api/fines/mark-paid`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
